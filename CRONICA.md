...
| 5   | POST   | `/auth/forgot-password` | Enviar email de recuperación de contraseña |
...
Envía un email al usuario con un link para restablecer su contraseña. Utiliza la API `executeActionsEmail` de Keycloak con la acción `UPDATE_PASSWORD`.

> **Requisito:** Keycloak debe tener SMTP configurado en Admin Console → Realm → Email → SMTP Settings.

**Request Body:** `ForgotPasswordRequestDTO`

| Campo   | Tipo   | Validación            | Descripción       |
| ------- | ------ | --------------------- | ----------------- |
| `email` | String | `@NotBlank`, `@Email` | Email del usuario |
...
> **Nota de seguridad:** Siempre retorna 200 independientemente de si el email existe o no, para evitar enumeración de usuarios.

**Para el frontend (Ionic):**

```typescript
// forgot-password.page.ts
forgotPassword() {
  this.authService.forgotPassword(this.email).subscribe({
    next: (msg) => this.showToast(msg),
    error: (err) => this.showToast('Error al enviar el correo')
  });
}
```
...
| 16  | PUT    | `/user/providers/{id}/onboarding/banking` | Completar onboarding bancario         |
...
##### PUT `/user/providers/{id}/onboarding/banking` — Completar onboarding bancario

Endpoint dedicado para que un proveedor complete su información bancaria y pueda recibir pagos semanales (settlements). Valida que el usuario tenga rol `PROVIDER` y que no haya completado el onboarding previamente.

**Path Variables:**

| Nombre | Tipo   | Descripción                      |
| ------ | ------ | -------------------------------- |
| `id`   | String | ID del proveedor (Keycloak UUID) |

**Request Body:** `BankingOnboardingRequestDTO`

| Campo               | Tipo         | Validación  | Descripción                              |
| ------------------- | ------------ | ----------- | ---------------------------------------- |
| `bankName`          | String       | `@NotBlank` | Nombre del banco                         |
| `bankAccountType`   | String       | `@NotBlank` | Tipo de cuenta (`AHORROS` / `CORRIENTE`) |
| `bankAccountNumber` | String       | `@NotBlank` | Número de cuenta bancaria                |
| `documentType`      | DocumentType | `@NotBlank` | Tipo de documento (`CC` / `NIT` / `CE`)  |
| `documentNumber`    | String       | `@NotBlank` | Número de documento del titular          |

**Response:** `200 OK` — `ProviderResponseDTO` con `onboardingCompleted: true`

**Errores posibles:**

| Código | Causa                                         |
| ------ | --------------------------------------------- |
| `400`  | El usuario no tiene rol de proveedor          |
| `400`  | El onboarding ya fue completado anteriormente |
| `404`  | Usuario o perfil de proveedor no encontrado   |

> **Nota para el frontend:** Este endpoint se llama una sola vez durante el primer inicio del proveedor. Si el proveedor ya completó el onboarding, el endpoint retorna error 400. El campo `onboardingCompleted` en `ProviderResponseDTO` indica el estado actual.

**Flujo en el frontend (Ionic):**

```typescript
// onboarding-banking.page.ts
completeOnboarding() {
  const dto = {
  ...
  this.providerService.completeBankingOnboarding(this.userId, dto).subscribe({
    next: (res) => {
      if (res.onboardingCompleted) {
        this.router.navigate(['/provider/dashboard']);
      }
    },
    error: (err) => this.showToast(err.error || 'Error al completar el onboarding')
  });
}
```
